...
Sprint 2 adds a deterministic synthetic dataset and read-only tool layer under the same safety model. It still does not build or call an LLM agent. Future agents will call typed, validated Python tools that read local synthetic JSON only.
